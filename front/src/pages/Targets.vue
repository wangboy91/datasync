<template>
  <div>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑目标库' : '新增目标库'" width="500px">
...
import { getTargets, createTarget, updateTarget, testTarget, syncSchema } from "../services/api";
...
const testConnection = async (data: any) => {
  ...
  testing.value = true;
  try {
    const res = await testTarget(data);
    if (res.ok) {
...
const syncDbSchema = async (row: any) => {
  ...
  row.syncing = true;
  try {
    await syncSchema({ targetId: row.id });
...
const save = async () => {
  try {
    if (isEdit.value) {
      ...
      await createTarget({ ...form.value, id: crypto.randomUUID(), status: 'disconnected' });
    }
